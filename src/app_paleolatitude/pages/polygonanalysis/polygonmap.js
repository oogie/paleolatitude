...
export const comp = {
    ...
    oninit: function (vnode) {
        ...
        vnode.state.polygons = vnode.attrs.polygons
    ...
    view: function (vnode) {
        ...
        return (
            <div class="comp polygonmap">
                <div id={vnode.state.id} style={(vnode.attrs.width ? `width: ${vnode.attrs.width}; ` : "") + (vnode.attrs.height ? ` height: ${vnode.attrs.height}; ` : "")}/>
                {vnode.attrs.controls === false ? "" : (
                    <button class='btn_icon export_button' onclick={() => {e.redraw = false; exportToPNG(vnode);}}>
...
function initMap(vnode) {
    ...
    vnode.state.maplayers = {
        ...
        layer_polygon: setupPolygonLayer(vnode),
        layer_polygonlabels: setupPolygonLabelsLayer(vnode)
    }
    ...
    const map = new ol.Map({
        layers: [
            ...
            vnode.state.maplayers.layer_polygon,
            vnode.state.maplayers.layer_polygonlabels
        ],
        ...
        view: new View({
            ...
            extent: vnode.state.projection.extent.map((n) => {return n*3}),
...
function setupPolygonLayer(vnode) {
    const style_path = new Style({
        stroke: new Stroke({
            color: '#000000',
            width: 1
...
function setupPolygonLabelsLayer(vnode) {
    const style_label = new Style({
        text: new Text({
            textAlign: 'center',
            textBaseline: 'bottom',
...
function setupEvents(vnode) {
    ...
    let debouncedPointermove = helpers.general.debounce(function(e) {
        prevLoc = e.pixel;
        if (highlighted.length > 0) {
            highlighted.forEach((f) => {f.setStyle(f.values_.isActiveOverlap ? overlapStyle : undefined)});
            highlighted = [];
        }

        vnode.state.map.forEachFeatureAtPixel(e.pixel, function (f) {
            if (f.values_.plate !== undefined) {
                highlighted.push(f);
    ...
    vnode.state.map.on('pointermove', debouncedPointermove);

    vnode.state.map.on('click', function (e) {
        let clicked = [];
        vnode.state.map.forEachFeatureAtPixel(e.pixel, function (f) {
            if (f.values_.plate !== undefined) {
                clicked.push(f.values_.plate)
            }
            return false;
        });

        vnode.attrs.onclick?.(e, clicked);
...
function updateLocations(vnode) {
    ...
    if (vnode.attrs.polygons) {
        drawPolygons(vnode.attrs.polygons, vnode);
    }
    if (vnode.attrs.activeOverlap) {
        drawOverlap(vnode.attrs.activeOverlap, vnode)
    }
...
function clearLocations(vnode) {
    ...
    vnode.state.maplayers.layer_polygon.getSource().clear();
    vnode.state.maplayers.layer_polygonlabels.getSource().clear();
}
...
function drawLocations(locations, isPath, vnode) {
    ...
    locations.forEach((location, i, allLocations) => {
        ...
        // let overlay = vnode.state.map.getOverlayById("location" + location.age);
        //
        // if (overlay === null) {
        ...
        //todo, check with Bram if his code always produces southern poles.
        //if not, check if plat is negative, then invert it and add 180 to plon, otherwise do nothing
        const coord = new Point(
            [longitude, latitude]
            // latitude < 0 ?
                // [(longitude+180) % 180, latitude*-1] :
                // [(longitude+180) % 180, latitude]
...
function drawPolygons(plates, vnode) {
    const overlapStyle = new Style({
        stroke: new Stroke({
            color: '#000000',
            width: 1
    ...
    plates.forEach((plate) => {
        let isActiveOverlap = vnode.attrs.activeOverlap?.shapeA.id === plate.id || vnode.attrs.activeOverlap?.shapeB.id === plate.id;

        let feature_polygon = new Feature({
            geometry: new Polygon(plate.rings).transform('EPSG:4326', vnode.state.projection.name),
            plate: plate,
            isActiveOverlap: isActiveOverlap
        });
        if (isActiveOverlap) {
            feature_polygon.setStyle(overlapStyle);
        }
        vnode.state.maplayers.layer_polygon.getSource().addFeature(feature_polygon);

        plate.rings.forEach((ring) => {
            const poleOfInaccessibility = polylabel([ring], 1);

            let zIndex = 0;
            let feature_label = new Feature({geometry: new Point(poleOfInaccessibility).transform('EPSG:4326', vnode.state.projection.name), label: `${plate.id} - ${plate.name}`, zIndex: zIndex});
            vnode.state.maplayers.layer_polygonlabels.getSource().addFeature(feature_label);
        });
    })

}

function drawOverlap(overlap, vnode) {
    const overlapStyle = new Style({
        stroke: new Stroke({
            color: '#880000',
            width: 4
    ...
    let feature_polygon = new Feature({
        geometry: new Polygon(overlap.intersection.polygon).transform('EPSG:4326', vnode.state.projection.name),
    });
    feature_polygon.setStyle(overlapStyle);
    vnode.state.maplayers.layer_polygon.getSource().addFeature(feature_polygon);

    vnode.state.map.getView().fit(feature_polygon.getGeometry().getExtent(), {padding: [30, 30, 30, 30], maxZoom: 8});


    overlap.intersection.polygon.forEach((ring) => {
        const poleOfInaccessibility = polylabel([ring], 1);

        console.log(overlap)

        let zIndex = 1;
        let area = helpers.geo.displayArea(helpers.geo.areaOf(ring));
        let feature_label = new Feature({geometry: new Point(poleOfInaccessibility).transform('EPSG:4326', vnode.state.projection.name), label: `Overlap of ${area}: ${overlap.shapeA.id} & ${overlap.shapeB.id}`, zIndex: zIndex});
        vnode.state.maplayers.layer_polygonlabels.getSource().addFeature(feature_label);
    });
}
...
function exportToPNG(vnode) {
    ...
    map.once('rendercomplete', function () {
        ...
        link.download = vnode.attrs.exportName ?? "APWP-online.org map.png";
        link.click();
    });
    map.renderSync();
}
